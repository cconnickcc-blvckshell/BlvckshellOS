"""Paired experiment gate — Case retrieval (J12)."""
...
def _case_control(domain: str) -> JudgmentProfile:
...
def _case_treatment(domain: str) -> JudgmentProfile:
    return JudgmentProfile(
        domain=domain,
        harm_guard_enabled=False,
        safe_divergence_enabled=True,
...
async def _combined_result(*archetypes) -> PairedExperimentResult:
    ...
    for arch in archetypes:
        result = await run_paired_experiment(
            arch,
            control_profile=_case_control(arch.domain),
            treatment_profile=_case_treatment(arch.domain),
            runs=10,
            seed_poor_outcomes=True,
        )
    ...
    return PairedExperimentResult(
        archetype_id="case_retrieval_combined",
...
@pytest.mark.asyncio
async def test_case_retrieval_gate_passes_federation() -> None:
    """Case retrieval passes paired promotion gate."""
